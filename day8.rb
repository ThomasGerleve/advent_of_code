input = "badc bd dbeaf cfdbge dfb cfbdea efbag edcfgab dcafe degfca | eacfd acdfbe cbdegf fcbaedg
...
input2 = input.split("\n")
input3 = input2.map { |row| row.split(" | ") }
input4 = input3.flatten
input5 = input4.map { |str| input4.index(str).even? ? str.split(" ").sort_by(&:length).join(" ") : str }
# input4 even = unique signal patterns
usp = input5.select.each_with_index { |_, i| i.even? }
# input4 odd = four digit output value
fdov = input5.select.each_with_index { |_, i| i.odd? }

# part 1
# how_many = 0
# fdov.each do |str|
#   arr = str.split(" ")
#   that_many = arr.each do |digit|
#     p digit.chars.sort.join
#     p digit.length
#     how_many += 1 if digit.length == 2
#     how_many += 1 if digit.length == 3
#     how_many += 1 if digit.length == 4
#     how_many += 1 if digit.length == 7
#   end
# end
# p how_many
...
# 3 digits: 7 a || b ||      d!
# 4 digits: 4 a || b ||           e || f
# 5 digits: 2 a ||      c || d ||      f || g
#           3 a || b || c || d ||      f
#           5      b || c || d || e || f
# 6 digits: 0 a || b || c || d || e ||      g
#           6      b || c || d || e || f || g
#           9 a || b || c || d || e || f
# 7 digits: 8 a || b || c || d || e || f || g
sum = 0
input3.each do |pair|
  usps = pair[0].split(" ").sort_by(&:length)
  usp_hash = {}
  usps.each do |usp|
    sorted = usp.chars.sort.join
    usp_hash["1"] = sorted if usp.length == 2
    # p usp_hash["1"]
    usp_hash["7"] = sorted if usp.length == 3
    usp_hash["4"] = sorted if usp.length == 4
    usp_hash_difference = usp_hash["4"].chars - usp_hash["1"].chars if usp_hash["4"]
    # p usp_hash_difference
    usp_hash["8"] = sorted if usp.length == 7
    if usp.length == 5
      if (sorted.chars & usp_hash["1"].chars).length == 2
        usp_hash["3"] = sorted
      elsif (sorted.chars & usp_hash_difference).length == 2
        usp_hash["5"] = sorted
      else
        usp_hash["2"] = sorted
      end
    ...
    if usp.length == 6
      if (sorted.chars & usp_hash["4"].chars).length == 4
        usp_hash["9"] = sorted
      elsif (sorted.chars & usp_hash["1"].chars).length == 2
        usp_hash["0"] = sorted
      else
        usp_hash["6"] = sorted
      end
  ...
  p usp_hash
  fdovs = pair[1].split(" ")
  sorted_fdovs = fdovs.map { |fdov| fdov.chars.sort.join }
  output = []
  sorted_fdovs.each do |fdov|
    output << usp_hash.index(fdov)
  end
  solution = output.join.to_i
  sum += solution
end
p sum
